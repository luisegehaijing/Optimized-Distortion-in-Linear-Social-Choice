...
def parse_experiment_log(log_content,parsed_data=[]):
    ...
    for line in lines:
        ...
        if stripped_line.startswith("Rule:"):
            ...
            if current_rule == "uniform":
                current_metrics_for_rule['running_time'] = 0.0 # Special case for uniform rule, as it has no running time in the log
        ...
        if current_rule and current_n is not None:
            ...
            if all(k in current_metrics_for_rule for k in ['empirical_distortion', 'theoretical_distortion', 'running_time']):
                parsed_data.append({
                    'n': current_n,
                    'm': current_m,
                    'd': current_d,
...
if __name__ == "__main__":
    ...
    log_file_path_1 = 'new_results_m_1222.1099395' # <--- IMPORTANT: Change this to your actual log file path
    log_file_path_2 = 'new_results_m_32.1099396' 
    log_file_path_3 = 'new_results_m_42.1099397' 
    log_file_path_4 = 'new_results_m_52.1099399'
    log_file_path_5 = 'new_results_m_all.1098631'
    

    file_paths = [log_file_path_1, log_file_path_2, log_file_path_3, log_file_path_4, log_file_path_5]
    parsed_data= []
    for file_path in file_paths:
        try:
            with open(file_path, 'r') as f:
                log_data = f.read()
                parsed_data=parse_experiment_log(log_data, parsed_data)
    ...
    plt.tight_layout()
    plt.savefig('m_emprical_distortion.png')
    ...
    leg_lines[7].set_linestyle(":")
    plt.tight_layout()
    plt.savefig('m_theoretical_distortion.png')
    ...
    leg_lines[7].set_linestyle(":")
    plt.tight_layout()
    plt.savefig('m_running_time.png')
